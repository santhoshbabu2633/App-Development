...
function His() {
  const [his, setHisStories] = useState([]);
  ...
  useEffect(() => {
    ...
    axios
      .get('http://localhost:8080/api/story/byGenre/history', axiosConfig)
      .then((response) => {
        setHisStories(response.data);
    ...
    <div>
      <div className="nonbg">
        <div className="navbar">
          ...
          <img className="mbas" src="ml.png" alt="Logo" />
          <div className="cred">
        ...
        <ul>
          {his.map((story) => (
            <ul key={story.storyid}>
              ...
              <div className="ms1">
                {renderEditorContent(story.content)}
              </div>
