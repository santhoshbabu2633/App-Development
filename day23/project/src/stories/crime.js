...
function Crime() {
  const [crime, setCrimeStories] = useState([]);
  ...
  useEffect(() => {
    ...
    axios
      .get('http://localhost:8080/api/story/byGenre/crime', axiosConfig)
      .then((response) => {
        setCrimeStories(response.data);
        const initialLikeCounts = {};
        const initialDislikeCounts = {};
        response.data.forEach((story) => {
          initialLikeCounts[story.storyid]=story.likeCount;
          initialDislikeCounts[story.storyid]=story.likeCount;
        });
