...
function Nonfic() {
  const [nonfic, setNonficStories] = useState([]);
  ...
  useEffect(() => {
    ...
    axios
      .get('http://localhost:8080/api/story/byGenre/NON-FICTION', axiosConfig)
      .then((response) => {
        setNonficStories(response.data);
      ...
      .catch((error) => {
        console.error('Error fetching non-fictional stories:', error);
      });
  }, [token,userRatings]);
